...
class ContentProjectViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        if self.request.user.has_special_access or self.request.user.is_staff:
            return ContentProject.objects.all()  # Fix: Refer to ContentProject, not Project
        return ContentProject.objects.filter(team_members=self.request.user)  # Fix: Refer to ContentProject
    ...
    def update(self, request, pk=None):
        project = get_object_or_404(ContentProject, pk=pk)  # Fix: Refer to ContentProject, not Project
        if request.user.has_special_access or request.user.is_staff:
            serializer = self.serializer_class(project, data=request.data, partial=True)
            if serializer.is_valid():
                serializer.save()
