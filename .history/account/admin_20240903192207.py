...
class CustomUserAdmin(admin.ModelAdmin):
    list_display = ('phone_number', 'first_name', 'last_name', 'is_active', 'is_staff', 'has_special_access')
...
def approve_registration(modeladmin, request, queryset):
    for pending_registration in queryset:
        user = CustomUser.objects.create_user(phone_number=pending_registration.phone_number, password=pending_registration.password, first_name=pending_registration.first_name, last_name=pending_registration.last_name)
